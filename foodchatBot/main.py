...
def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
        ...
        if order_id == -1:
            ...
            order_details = ""
            for food_item, quantity in order.items():
                price_per_item = db_helper.get_food_item_price(food_item)
                total_price_for_item = price_per_item * quantity
                order_details += f"{food_item}: {quantity} x {price_per_item} = {total_price_for_item}\n"
...
def add_to_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        fulfillment_text = "Sorry I didn't understand. Can you please specify the quantities and food items that is present in the menu?"
    else:
...
def track_order(parameters: dict, session_id: str):
    ...
    if order_id:
        ...
        fulfillment_text = "Order ID not provided. Please provide your order ID to track the order."
